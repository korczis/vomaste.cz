...
| ID | Titul | Scope (soubory/sekce) | Branch | Owner | Stav | Závislosti | Akceptace |
|----|-------|-----------------------|--------|-------|------|------------|-----------|
| T-019 | Fáze 5 — registry tvrzení/zdrojů/evidence/mezer: dense tabulky s toolbar filtry, URL stav (§ 8), inspector master-detail (§ 7), coverage matrix, source families v registru zdrojů. **Řádek byl zastaralý — práce už dávno na masteru.** Ověřeno 2026-08-08 (`git merge-base --is-ancestor 2e60eeaa master` → true): `templates/macros/table.html` (`table::advanced_table`) + `assets/js/modules/table-filter.js` implementují plný toolbar (fulltext, sloupcové řazení, chip/select facety, URL stav, sloupcová viditelnost, CSV/JSON export) a `assets/js/modules/url-state.js` stav v URL; inspector master-detail hotov (`2e60eeaa`). Použito v dossier-claims/sources/gaps/cases-index.html, entities-index.html, source-catalog-index.html, tooling-index.html, dossier-evidence.html. Vědomě chybí jen stránkování (zdokumentováno v table-filter.js: až přes tisíc řádků) a `templates/entity-dossier-registry.html` (per-entity dossier registry, používané petr-macinka/filip-turek) zůstává na statických registry-card gridech, ne na table:: — to je teď reálný zbytek scope, patří pod T-013. | templates/dossier-*-index.html, entity-dossier-*.html, assets/js/modules/table-filter.js, macros/table.html | – | ORCH | merged (f9260a7c, 5a2aba4b, 92e3fc37, 5d065025, 6ebb470b, 2e60eeaa — vše na masteru) | – | filtry reprodukovatelné URL; detail bez ztráty kontextu; rozlišení zdroje vs. nezávislé rodiny |
...
| ID | Titul | Scope (soubory/sekce) | Branch | Owner | Stav | Závislosti | Akceptace |
|----|-------|-----------------------|--------|-------|------|------------|-----------|
| T-012 | Advanced application shell — shell primitiva. Fáze C. **Hotovo.** Sekundární sidebar (0e17092/8eb5af2: druhý dokovaný sloupec z T-011 dat, desktop-only, 0 overflow ověřeno testem, WCAG AA kontrast oprava) + mobile bottom nav (viz T-012b). Topbar/primární sidebar/mobile drawer už existovaly (viz ADR audit). Context panel je vědomě mimo tuto fázi — T-013 už jmenuje "record-detail" a T-019 postavil funkční master-detail inspector pro registrové tabulky; generický context panel by ho duplikoval, lépe rozhodnout v T-013/T-019 s tímto vzorem před očima. | templates/base.html, templates/partials/app-shell/** | task/T-012 | ORCH | merged (b3ce5ebd) | T-011 | 0 horizontální overflow na testovaných viewportech, focus trap/return funkční, no-JS fallback zachován |
| T-012b | Mobile bottom navigation pro T-012 — kompaktní fixed bottom bar (3 cíle: domů/dossiery/mapa + Menu) na malých viewportech, sdílí `data/generated/navigation.json` s primárním sidebarem (žádný hardcoded slug); Menu button je STEJNÝ Flowbite drawer trigger jako hamburger v navbaru (jeden drawer, dva spouštěče). `#main` dostává `pb-16 md:pb-0`. | `templates/base.html`, nová `templates/partials/app-shell/bottom-nav.html`, `tests/e2e/bottom-nav.spec.mjs` | task/T-012 | ORCH | merged (b3ce5ebd, `--no-ff`, po nezávislém ověření v izolovaném worktree) | T-012 | e2e `bottom-nav.spec.mjs` 4/4 (2 skip = desktop-only test); plný `npm run build` zelený jak v izolovaném worktree (324.1s), tak znovu na masteru po merge (321.0s); a11y sweep (44 desktop + 16 mobile testů) beze změny v chování, 0 nových overflow porušení; push/deploy potvrzen |
