...
### Token, který vypadal jako kauza a žádná není

Jeden token v rejstříkovém záznamu se dal přečíst jako „padělky" (padělané
zboží). To čtení bylo **mylné**: jde o název obytné ulice, který agregátory
uvádějí jako součást **adresy bydliště**.

Konkrétní znění se zde **neuvádí** — ani ulice, ani číslo popisné. Adresa
bydliště se podle `AGENTS.md` (brána 6, minimalizace dat) a
`.claude/rules/authorization.md` do repozitáře nepřebírá v žádné podobě,
a to platí i pro interní rešeršní poznámku: veřejný Git nezapomíná.

Zaznamenáno je jen to, co má trvalou hodnotu — **že tenhle signál byl planý
poplach**. Adresa není kauza.
