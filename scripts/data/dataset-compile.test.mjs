...
test("check nad reálným data/dossiers projde (50 balíčků, vlastimil-valek přidán)", async () => {
  ...
  // 50 = předchozích 49 + autorizovaný dossier vlastimil-valek.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 50 dossier balíčků")), lines.join("\n"));
