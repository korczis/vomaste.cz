...
test("check nad reálným data/dossiers projde (43 balíčků, ivan-bartos přidán)", async () => {
  ...
  // 43 = předchozích 42 + autorizovaný dossier ivan-bartos.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 43 dossier balíčků")), lines.join("\n"));
