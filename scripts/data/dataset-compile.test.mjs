...
test("check nad reálným data/dossiers projde (27 balíčků včetně martin-kupka)", async () => {
  ...
  // 27 = předchozích 26 + autorizovaný dossier martin-kupka.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 27 dossier balíčků")), lines.join("\n"));
