...
test("check nad reálným data/dossiers projde (44 balíčků, katerina-stojanova přidána)", async () => {
  ...
  // 44 = předchozích 43 + autorizovaný dossier katerina-stojanova.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 44 dossier balíčků")), lines.join("\n"));
