...
test("check nad reálným data/dossiers projde (46 balíčků, marian-jurecka přidán)", async () => {
  ...
  // 46 = předchozích 45 + autorizovaný dossier marian-jurecka.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 46 dossier balíčků")), lines.join("\n"));
