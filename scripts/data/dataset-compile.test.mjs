...
test("check nad reálným data/dossiers projde (45 balíčků, jan-grolich přidán)", async () => {
  ...
  // 45 = předchozích 44 + autorizovaný dossier jan-grolich.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 45 dossier balíčků")), lines.join("\n"));
