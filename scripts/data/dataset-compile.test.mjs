...
test("check nad reálným data/dossiers projde (49 balíčků, jan-jakob přidán)", async () => {
  ...
  // 49 = předchozích 48 + autorizovaný dossier jan-jakob.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 49 dossier balíčků")), lines.join("\n"));
