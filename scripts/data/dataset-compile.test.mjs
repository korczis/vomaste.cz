...
test("check nad reálným data/dossiers projde (42 balíčků, olga-richterova přidána)", async () => {
  ...
  // 42 = předchozích 41 + autorizovaný dossier olga-richterova.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 42 dossier balíčků")), lines.join("\n"));
