...
test("check nad reálným data/dossiers projde (137 balíčků včetně miroslavy-kopicove)", async () => {
  ...
  // 32 = předchozích 31 + autorizovaný dossier eva-decroix.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 137 dossier balíčků")), lines.join("\n"));
