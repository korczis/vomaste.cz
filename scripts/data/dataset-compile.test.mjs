...
test("check nad reálným data/dossiers projde (28 balíčků včetně petr-fiala)", async () => {
  ...
  // 28 = předchozích 27 + autorizovaný dossier petr-fiala.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 28 dossier balíčků")), lines.join("\n"));
