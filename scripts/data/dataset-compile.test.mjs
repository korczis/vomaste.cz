...
test("check nad reálným data/dossiers projde (39 balíčků, vit-rakusan přidán)", async () => {
  ...
  // 39 = předchozích 38 + autorizovaný dossier vit-rakusan.
  const lines = [];
  ...
  assert.ok(lines.some((l) => l.includes("Načteno 39 dossier balíčků")), lines.join("\n"));
