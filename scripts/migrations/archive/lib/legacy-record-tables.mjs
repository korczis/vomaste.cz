// LEGACY ORACLE (T-028 Phase G follow-up): frozen pre-Phase-G copy of
// scripts/dossier/lib/record-tables.mjs (from commit 55562e0^), kept ONLY
// as the migrator's independent cross-check parser. After Phase G the
// live record-tables.mjs projects the compiled canonical model, so using
// it as the migration oracle would be circular (it would compare new
// content against the OLD canonical dataset). This copy still parses
// content/** front matter directly. Do not "modernize" it — its value is
// exactly that it is a second, independent implementation. Remove in
// Phase H together with the migrator itself.
