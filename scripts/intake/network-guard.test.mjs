...
// §28). Phase 2/3 must work completely offline; Phase 4's
// scripts/intake/preflight/ subsystem is the ONE deliberate exception —
// and even there, network primitives are pinned to exactly the two
// designated transport-adapter modules (request-once.mjs,
// resolve-hostname.mjs), never scattered across policy/orchestration
// code. Deliberately narrow (exact tokens, not a broad heuristic) to
// stay maintainable and avoid false positives on unrelated words.
...
const ALLOWED_NETWORK_MODULES = new Set([join(PREFLIGHT_DIR, "request-once.mjs"), join(PREFLIGHT_DIR, "resolve-hostname.mjs")]);
...
test("no module under scripts/intake/ (excluding preflight's two designated transport adapters and all test-only files) contains a network or shell-exec primitive", () => {
...
test("network primitives inside scripts/intake/preflight/ exist in exactly the two designated transport-adapter modules, nowhere else", () => {
...
test("the two designated transport-adapter modules actually exist and actually do use the network primitives they're allow-listed for (the allowlist isn't vacuous)", () => {
