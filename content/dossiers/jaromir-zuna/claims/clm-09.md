+++
title = "CLM-09"
description = "Opoziční politici nezveřejnění rozhovoru charakterizovali jako cenzuru prezidenta: Josef Flek (STAN) mluvil o 'bezprecedentním pokusu o umlč"
template = "dossier-claim.html"
weight = 9

[extra]
dossier = "jaromir-zuna"
record_type = "claim"
lang = "cs"
clm_id = "CLM-09"
status = "status-quote"
status_label = "CITACE"
summary = "Opoziční politici nezveřejnění rozhovoru charakterizovali jako cenzuru prezidenta: Josef Flek (STAN) mluvil o 'bezprecedentním pokusu o umlčení hlavy státu', Martin Kupka (ODS) o 'cenzuře v nejtvrdší podobě' a Matěj Ondřej Havel (TOP 09) žádal Zůnovu rezignaci."
sources = ["SRC-06"]
subjects = ["zuna"]
+++

Viz plné znění, kontext a sousední tvrzení v [hlavním přehledu](@/dossiers/jaromir-zuna/_index.md#registr-tvrzeni).
