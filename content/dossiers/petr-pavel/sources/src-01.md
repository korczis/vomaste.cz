+++
# GENERATED FILE. DO NOT EDIT. Zdroj: data/dossiers/** — regeneruje `npm run data:build`.
title = "SRC-01 — Deník.cz (VLTAVA LABE MEDIA)"
description = "Prezident Pavel zdůvodňuje odmítnutí jmenovat Turka ministrem; Turek oznamuje žalobu na ochranu osobnosti."
template = "dossier-source.html"
weight = 1

[extra]
generated = true
record_id = "https://vomaste.cz/id/dossiers/petr-pavel/sources/SRC-01"
view_model = "generated/views/dossiers/petr-pavel/sources/src-01.json"
dossier = "petr-pavel"
record_type = "source"
lang = "cs"
src_id = "SRC-01"
+++
Prezident Petr Pavel zdůvodnil odmítnutí jmenovat Filipa Turka ministrem
životního prostředí: uvedl, že Turek opakovaně prokazoval nedostatek
respektu k českému právnímu řádu, opakovaně glorifikoval nebo přinejmenším
zlehčoval nacistické Německo jako jeden z nejhorších totalitních režimů
20. století, a vážně zpochybňoval důstojnost a rovnost žen a příslušníků
různých menšin. Turek na to reagoval oznámením, že podá žalobu na ochranu
osobnosti a bude žádat prezidentovu omluvu.

Tentýž zdroj a tentýž text už dokládá
[CLM-38](@/dossiers/macinka-turek/claims/clm-38.md) v dossieru
Filipa Turka — je zde znovupoužit jako vlastní, fyzicky samostatný
záznam tohoto dossieru, protože fakt se přímo týká i prezidenta Pavla,
ne jen Turka.
