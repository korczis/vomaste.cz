+++
# GENERATED FILE. DO NOT EDIT. Zdroj: data/dossiers/** — regeneruje `npm run data:build`.
title = "Martin Pavlík"
description = "Neutrální, zdroji doložený přehled veřejných rejstříkových vazeb Martina Pavlíka; bez směšování osob stejného jména."
template = "entity-dossier.html"

[extra]
generated = true
record_id = "https://vomaste.cz/id/dossiers/martin-pavlik"
view_model = "generated/views/dossiers/martin-pavlik/overview.json"
dossier = "martin-pavlik"
record_type = "dossier"
dossier_type = "entity"
lang = "cs"
updated = "2026-08-05"
reviewed_at = "2026-08-05"
+++
## Rozsah dossieru {#rozsah}

Tento dossier je záměrně založen jako čistý, minimální záznam. Je vyhrazen výhradně pro veřejné rejstříkové vazby Martina Pavlíka jednoznačně rozlišeného profilem uvedeným v autorizačním záznamu `AUTH-2026-08-05-MARTIN-PAVLIK`.

Publikováno je šest rejstříkových faktů: čtyři zapsané funkce a dva obchodní podíly. Tři z nich stojí na dvou nezávislých doloženích — primárním výpisu z veřejného rejstříku (ARES) a odvozeném rejstříkovém profilu —, zbylé tři zatím jen na primárním rejstříku. Profil v IT, schopnosti, jiné zaměstnání, věk, datum narození, bydliště a údaje o jmenovcích dossier nepřebírá. Nejasnost identity se eviduje jako mezera, nikdy se nedoplňuje odhadem.

## Registr tvrzení {#registr-tvrzeni}

| ID | Tvrzení | Stav | Zdroje |
|---|---|---|---|
| <a id="clm-01"></a>[CLM-01](@/dossiers/martin-pavlik/claims/clm-01.md) | Veřejný rejstřík (ARES) i rejstříkový profil Podnikatel.cz uvádějí Martina Pavlíka jako jednatele společnosti MEDIA PROJECT CZ s.r.o. (IČO 01529820) | <span class="status-badge status-corroborated">CORROBORATED</span> | [SRC-01](@/dossiers/martin-pavlik/sources/src-01.md), [SRC-02](@/dossiers/martin-pavlik/sources/src-02.md) |
| <a id="clm-02"></a>[CLM-02](@/dossiers/martin-pavlik/claims/clm-02.md) | Veřejný rejstřík (ARES) i rejstříkový profil Podnikatel.cz uvádějí Martina Pavlíka jako 1. místopředsedu spolku Bydlíme v Králově Poli, z.s. (IČO 02922703) | <span class="status-badge status-corroborated">CORROBORATED</span> | [SRC-01](@/dossiers/martin-pavlik/sources/src-01.md), [SRC-02](@/dossiers/martin-pavlik/sources/src-02.md) |
| <a id="clm-03"></a>[CLM-03](@/dossiers/martin-pavlik/claims/clm-03.md) | Veřejný rejstřík (ARES) i rejstříkový profil Podnikatel.cz uvádějí Martina Pavlíka jako jednatele společnosti HYDROPROGRESS, s.r.o. (IČO 04449461) | <span class="status-badge status-corroborated">CORROBORATED</span> | [SRC-01](@/dossiers/martin-pavlik/sources/src-01.md), [SRC-02](@/dossiers/martin-pavlik/sources/src-02.md) |
| <a id="clm-04"></a>[CLM-04](@/dossiers/martin-pavlik/claims/clm-04.md) | Veřejný rejstřík (ARES) uvádí Martina Pavlíka jako člena orgánu Nadačního fondu FIDUCIA (IČO 26228548). Zápis funkce je evidenční fakt, nikoli podezření z čehokoli | <span class="status-badge status-single">1 ZDROJ</span> | [SRC-02](@/dossiers/martin-pavlik/sources/src-02.md) |
| <a id="clm-05"></a>[CLM-05](@/dossiers/martin-pavlik/claims/clm-05.md) | Podle rejstříkového výpisu ARES je Martin Pavlík společníkem společnosti HYDROPROGRESS, s.r.o. (IČO 04449461) s obchodním podílem 80 %, vedle jednatelství. Vlastnictví podílu je evidenční fakt, nikoli podezření z čehokoli | <span class="status-badge status-single">1 ZDROJ</span> | [SRC-02](@/dossiers/martin-pavlik/sources/src-02.md) |
| <a id="clm-06"></a>[CLM-06](@/dossiers/martin-pavlik/claims/clm-06.md) | Podle rejstříkového výpisu ARES je Martin Pavlík společníkem společnosti MEDIA PROJECT CZ s.r.o. (IČO 01529820) s obchodním podílem 50 %, vedle jednatelství. Vlastnictví podílu je evidenční fakt, nikoli podezření z čehokoli | <span class="status-badge status-single">1 ZDROJ</span> | [SRC-02](@/dossiers/martin-pavlik/sources/src-02.md) |

## Metodické omezení

Uvedení role v rejstříkovém agregátoru samo o sobě nedokládá skutečný rozsah činnosti, rozhodování ani jakékoli pochybení. Dossier z těchto tří zápisů nic dalšího nevyvozuje.
