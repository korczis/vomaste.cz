+++
title = "GAP-04 — Majetkové a podnikatelské zájmy"
description = "Firemní vazby a Turkovy nemovitosti jsou zdokumentované; Macinkovy nemovitosti se veřejně dostupnými zdroji dohledat nepodařilo."
template = "dossier-gap.html"
weight = 4
aliases = ["/dossier/mezery/gap-04/"]

[extra]
subjects = ["macinka", "turek"]
dossier = "macinka-turek"
record_type = "gap"
lang = "cs"
gap_id = "GAP-04"
priority = "nízká"
checked = "2026-07-29"
claims = ["CLM-14", "CLM-15", "CLM-20", "CLM-22", "CLM-34"]
+++

Firemní a spolkové vazby obou politiků i Macinkova nepřiznaná
dvacetiprocentní účast v ukrajinské firmě GMR GAS UA LLC jsou
zdokumentovány (CLM-14, CLM-15, CLM-20, CLM-22). U Turka je zdokumentován
i pozemek v Praze-Dubči (se dvěma dodatečně zlegalizovanými nepovolenými
stavbami) a byt na Strahově koupený v dubnu 2026 za 18 mil. Kč na
hypotéku (CLM-34). Viz [hlavní přehled](@/dossiers/macinka-turek/_index.md#majetkove-podnikatelske-a-financni-vazby).

**Doplnění (dohledáno 2026-07-23)**: server [Extra.cz (2025-11-20)](https://www.extra.cz/filip-turek-zverejnil-majetkove-priznani-luxusni-vozy-za-desitky-milionu-drahe-hodinky-a-nemovitost-v-praze-e95a4)
cituje Turkovo tehdejší majetkové přiznání: „Co se týče nemovitostí,
Turek vlastní jen jednu: pozemek s chatou v Praze 10" — tedy k listopadu
2025 (před dubnovým nákupem bytu na Strahově) jen pozemek v Dubči, což
odpovídá dossieru a nic nerozporuje. Jeho trvalé bydliště bylo v té době
psané v nájemním bytě na Břevnově, ne ve vlastnictví. Žádný dohledaný
zdroj neuvádí u Turka další nemovitost nad rámec Dubče a Strahova.

**Doplnění (dohledáno 2026-07-29)**: přehled poprvé zkusil onen
„samostatný sběr z primárního registru" zmíněný níže — přímým programovým
dotazem (ne rešerší přes žurnalistiku) na primární rejstřík ARES
(ares.gov.cz) a na veřejné rozhraní ČÚZK. ARES nezávisle potvrdil přesnost
už citovaných rejstříkových údajů o firemních vazbách obou politiků
(IČO, sídla, stav „v likvidaci" u MEAS Consulting s.r.o. — SRC-23,
SRC-24) beze změny; nic nového k firemním vazbám to nepřineslo.

U ČÚZK se zpřesnil samotný důvod nedohledatelnosti: přímý programový
dotaz na `nahlizenidokn.cuzk.cz` potvrdil, že **veškeré** požadavky na
tento veřejný web — včetně prostého vstupu na úvodní stránku, bez
jakéhokoli zadaného jména — jsou přesměrovány na ochranu proti botům
(Radware Bot Manager), nikoli jen jmenné vyhledávání. Přesnější tedy je,
že přístup blokuje bezpečnostní infrastruktura celého veřejného rozhraní,
ne (jen) že by jmenné vyhledávání bylo cíleně vyřazené kvůli ochraně
osobních údajů. Věcný závěr se tím nemění.

**Co zůstává otevřené**: nemovitosti Petra Macinky. Veřejné rozhraní
katastru nemovitostí (ČÚZK) je pro tento přehled programově
nedostupné (viz výše) a žádná dohledaná žurnalistika se tématem
Macinkových nemovitostí konkrétně nezabývala. **To není zjištění, že by
Macinka nemovitosti neměl** — jde jen o to, že žádnou metodou dostupnou
tomuto přehledu (rešerše publikované žurnalistiky ani přímý programový
dotaz na ČÚZK) se je nepodařilo dohledat. Úplné vyřešení by vyžadovalo
placený/autorizovaný přístup k datům ČÚZK (ISKN/RÚIAN) mimo veřejné
webové rozhraní, viz [Co tento přehled nezkoumal](@/dossiers/macinka-turek/_index.md#co-tento-prehled-nezkoumal).
